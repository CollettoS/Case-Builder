...
import plugins.escalation_builder
...
def add_user():
    ...
    if role:
        ...
        case_entities_list.append({'type': 'Role', 'ent': role})


        print(case_entities_list)
        if user:
            line = 1
        else: 
            line = 2
...
def add_row(ent_type, value1="Static 1", value2="Static 2", editable_value=""):
    ...
    case_entities_list.append({'type': ent_type, 'ent': value1})
...
def get_alert_name():
    case_name = title_entry.get().strip()
    return case_name
...
esc_button = ctk.CTkButton(general_tab, text="Escalate Case", width=100, command=lambda: plugins.escalation_builder.run(case_entities_list, get_alert_name()), height=20, fg_color="#d97706", hover_color="#f59e0b", font=("Verdana", 12, "bold"))
esc_button.grid(row=20, column=1, padx=2, pady=2, sticky="ew")  # Top-left button
